...
describe('Comprehensive Sync System Test', function () {
    ...
    const PARTITION_DURATION = 30000; // 30 seconds
    ...
    it('should synchronize all nodes after initial block creation', async function () {
        try {
            ...
            console.info('Initial synchronization successful. All nodes at height:', heights[0]);
        } catch (error) {
    ...
    it('should handle network partition and resync', async function () {
        try {
            ...
            console.info('Network partitioned');

            await createBlocksInPartition(partition1, 5);
            await createBlocksInPartition(partition2, 3);
            console.info('Created blocks in both partitions');

            await waitForSync(partition1);
            await waitForSync(partition2);

            const height1 = partition1[0].getStatus().currentBlockHeight;
            const height2 = partition2[0].getStatus().currentBlockHeight;
            expect(height1).to.not.equal(height2, 'Partitions should have different heights');
            console.info(`Partition heights: ${height1} and ${height2}`);

            await reuniteNetwork(partition1, partition2);
            console.info('Network reunited');

            await waitForSync(nodes);

            const heights = nodes.map(node => node.getStatus().currentBlockHeight);
            expect(new Set(heights).size).to.equal(1, 'All nodes should have synced to the same height');
            expect(Math.max(...heights)).to.equal(height1, 'All nodes should have synced to the longest chain');
            console.info('Network resynced successfully. All nodes at height:', heights[0]);
        } catch (error) {
            console.error('Test failed:', error);
            throw error;
        }
    ...
    it('should sync a new node joining the network', async function () {
        try {
            const newNode = await factory.createNode(accounts[NUM_NODES], { role: 'validator', syncPort: 10000 + NUM_NODES });
            ...
            const existingNodeHeight = nodes[0].getStatus().currentBlockHeight;
            expect(newNodeHeight).to.equal(existingNodeHeight, 'New node should sync to the current blockchain height');
            console.info('New node synced successfully. Height:', newNodeHeight);
        } catch (error) {
            console.error('Test failed:', error);
            throw error;
        }
