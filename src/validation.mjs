...
export class BlockValidation {
    ...
    static async fullBlockTxsValidation(blockData, utxoCache, memPool, workers, useDevArgon2 = false) {
        ...
        /*while (i < blockData.Txs.length) {
            for (const worker of workers) {
                if (workersPromises[worker.id]) { continue; } // not ready yet

                const tx = blockData.Txs[i];
                if (tx === undefined) { break; } // no more txs to validate
        ...
        console.log(`[VALIDATION] Multi thread ${blockData.Txs.length} txs validated in ${Date.now() - multiThreadStart} ms`);
        console.log(`[VALIDATION] Fast treated txs: ${fastTreatedTxs}`);
        /*while (i < blockData.Txs.length) { // Working but can be optimized
            const promises = [];
            for (const worker of workers) {
                const tx = blockData.Txs[i];
                const specialTx = i < 2 ? Transaction_Builder.isMinerOrValidatorTx(tx) : false;

                promises.push(worker.addressOwnershipConfirmation(
                    involvedUTXOs,
                    tx,
                    allImpliedKnownPubkeysAddresses,
                    useDevArgon2,
        ...
        //#endregion - MULTI THREADING VALIDATION_v2

        //#region - MULTI THREADING VALIDATION_v1
        // PARTIAL VALIDATION
        /*const allImpliedKnownPubkeysAddresses = {};
        for (let i = 0; i < blockData.Txs.length; i++) {
            const tx = blockData.Txs[i];
        ...
        for (const worker of workers) { worker.reset(); }
        const nbOfIdleWorkers = () => workers.filter(worker => worker.state === 'idle').length;

        let taskSent = 0; // just to be sure
        let validatedTxCount = 0;
        let i = 0;
        let invalidTx = false;
        while (validatedTxCount < blockData.Txs.length && !invalidTx) {
            while (nbOfIdleWorkers() === 0) { await new Promise(resolve => setTimeout(resolve, 10)); }
           // while (workers.findIndex(worker => worker.state === 'idle') === -1) { await new Promise(resolve => setTimeout(resolve, 10)); }
            //const idleWorker = workers.find(worker => worker.state === 'idle');
            //if (!idleWorker) { await new Promise(resolve => setTimeout(resolve, 10)); continue; }
            const idleWorkers = workers.filter(worker => worker.state === 'idle');
            for (const idleWorker of idleWorkers) {
                if (idleWorker.isValid === true) { 
                    validatedTxCount++;
                    for (let [pubKeyHex, address] of Object.entries(idleWorker.discoveredPubKeysAddresses)) {
                        allDiscoveredPubKeysAddresses[pubKeyHex] = address;
                    }
                    idleWorker.isValid = null;
                    idleWorker.state = 'idle';
                }
                if (idleWorker.isValid === false) {
                    invalidTx = true; // useless ATM because we throw an error
                    throw new Error(`Invalid transaction: ${tx.id}`);
                }

                if (validatedTxCount === blockData.Txs.length) { break; }
                // not assign task if we spent all txs tasks
                if (i >= blockData.Txs.length) { continue; }

                const tx = blockData.Txs[i];
                const specialTx = i < 2 ? Transaction_Builder.isMinerOrValidatorTx(tx) : false;
                
                idleWorker.state = 'working';
                idleWorker.addressOwnershipConfirmation(
                    involvedUTXOs,
                    tx,
                    allImpliedKnownPubkeysAddresses,
                    useDevArgon2,
        ...
        if (invalidTx) { throw new Error('Invalid transaction'); }
        if (taskSent !== blockData.Txs.length) { throw new Error('Task sent !== blockData.Txs.length'); }
        console.log()*/

        //#endregion - MULTI THREADING VALIDATION_v1
