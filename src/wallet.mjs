...
export class Wallet {
     ...
     */
    constructor(masterHex, useDevArgon2 = false, poolSize = 4) {
        /** @type {string} */
    ...
    /**
     * Derives multiple accounts in parallel with controlled concurrency.
     * @param {number} nbOfAccounts - Number of accounts to derive.
     * @param {string} addressPrefix - Desired address prefix.
     * @param {number} concurrency - Maximum number of concurrent derivations.
     * @returns {Promise<Object>} - Derived accounts and average iterations.
     */
    async deriveAccounts(nbOfAccounts = 1, addressPrefix = "C", concurrency = 10) {
        ...
        // Generate an array of account indices to derive
        const accountIndices = [];
        for (let i = nbOfExistingAccounts; i < nbOfAccounts; i++) {
            accountIndices.push(i);
        }

        // Define the task for each account derivation
        const tasks = accountIndices.map(i => async () => {
    ...
    async loadOrCreateAccounts(params) {
        ...
        const { derivedAccounts } = await this.deriveAccounts(params.nbOfAccounts, params.addressPrefix, params.concurrency || 10);
    ...
    /**
     * Helper method to run asynchronous tasks with limited concurrency.
