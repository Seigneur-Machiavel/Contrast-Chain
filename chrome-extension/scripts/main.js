...
async function testWorker(mem = 2**20, workerNb = 1) {
	const timings = {init: 0, postMessages: 0, jobs: 0, checkpoint: performance.now()};
	...
	timings.init = performance.now() - timings.checkpoint;
	timings.checkpoint = performance.now();
	...
	for (let i = 0; i < workerNb; i++) {
		...
		//worker.postMessage({pass: '123456', salt: 'totosaltlong', time: 2, mem, parallelism: 8, hashLen: 32, type: 2});
		workers[i].postMessage({pass: '123456', salt: 'totosaltlong', time: 1, mem, parallelism: 1, hashLen: 32, type: 2});
		promises.push(workerPromise);
	}
	timings.postMessages = performance.now() - timings.checkpoint;
	timings.checkpoint = performance.now();

	const autoKillAfterMs = 1000000; // 2000;
	const cancelPromise = new Promise((resolve, reject) => { setTimeout(() => { resolve('Timeout!'); }, autoKillAfterMs); });

	let result = null;

	while (true) {
		// check if result is resolved
		const allPromisesRes = await Promise.race([
			Promise.all(promises),
			cancelPromise,
			new Promise(resolve => setTimeout(resolve, 100)) // Délai court pour vérifier régulièrement l'annulation
		  ]);

		if (allPromisesRes) { result = allPromisesRes; break; }
	}

	timings.jobs = performance.now() - timings.checkpoint;
	...
	console.log(`Worker test duration:
		init: ${timings.init.toFixed(2)}ms,
		post: ${timings.postMessages.toFixed(2)}ms,
		jobs: ${timings.jobs.toFixed(2)}ms`);
	return result;
}
...
/*async function heartBeat(delay = 1000) { // DEPRECATED
...
async function randomizeMnemonic(modal = eHTML.modals.inputMasterMnemonic, asPlaceholder = false) {
	...
	const nonHiddenInputs = Array.from(mnemonicGridInputs).filter((input) => !input.parentElement.classList.contains('hidden'));
	const mnemonicGenerator = new Mnemonic();
	const mnemonic = mnemonicGenerator.genRandomMnemonic(wordsList, nonHiddenInputs.length, false);
	...
	for (let i = 0; i < mnemonicGridInputs.length; i++) {
		...
		for (let j = 0; j < word.length; j++) { // simple typing animation
			input.placeholder += word.charAt(j);
			if (!asPlaceholder) { input.value += word.charAt(j); };
...
function extractMnemonicFromInputs(modal = eHTML.modals.inputMasterMnemonic) {
	...
	const result = { allWordsAreFilled: true, allWordsAreValid: true, mnemonic: new Mnemonic() };
	const mnemonic = [];
	for (let i = 0; i < mnemonicGridInputs.length; i++) {
		const input = mnemonicGridInputs[i];
	...
	result.mnemonic = new Mnemonic(mnemonic, bip, language);
...
function switchBtnsIfMnemonicGridIsFilled(modalWrapID = "masterMnemonicModalWrap", saveInTempDataIfValid = true) {
	...
	if (!modal) {
		...
		return { allWordsAreValid: false, mnemonic: new Mnemonic() };
	}
...
function initMnemoLinkBubbles(releaseBubbles = false, delayBeforeRelease = 600) {
	...
	for (let i = 0; i < monemoLinksInfos.length; i++) {
		...
		const label = monemoLinksInfos[i].label;
		const versionStr = monemoLinksInfos[i].versionStr;
		const mnemoBubbleObj = new MnemoBubble(label, versionStr, x, y);
...
function initMnemoLinkSVGs() {
	...
	if (mnemolinkLinks.length !== mnemoBubblesObj.length) {
		...
		for (let i = 0; i < mnemoBubblesObj.length; i++) {
			...
			const mnemoLinkSVG = new MnemoLinkSVG(svg, mnemoBubblesObj[i].x, mnemoBubblesObj[i].y, sequence);
...
eHTML.modals.inputMasterMnemonic.mnemonicGrid.addEventListener('click', (event) => {
	if (event.target.tagName === 'BUTTON') {
		...
		const rndWord = mnemonic.getRandomWord(wordsList);
		input.value = rndWord;
		input.classList.add('random');
		actualizeScore();
