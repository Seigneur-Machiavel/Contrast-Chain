...
class MnemoBubble {
	constructor(label, mnemoLinkerVersion, x = 0, y = 0) {
		...
		this.state = 'readyForDeciphering';
	}
	...
	async prepareBubbleToShow(label, fakeCipher = true) {
		if (this.state !== 'readyForDeciphering') { return; }
		if (this.label !== label) { console.error('label mismatch !'); return; }
		this.state = 'fakeDeciphering';
		...
		if (!this.element.classList.contains('showing')) { this.#abortDeciphering(); return; }
		this.element.appendChild(emptyDiv);

		const wordDivs = gridHtml.querySelectorAll('div');
		while (this.state === 'fakeDeciphering') {
			for (let i = 0; i < wordDivs.length; i++) {
				...
				for (let j = 0; j < newRndWord.length; j++) {
					...
					if (this.state !== 'fakeDeciphering' || this.label !== label) { break; }
				}
				if (this.state !== 'fakeDeciphering' || this.label !== label) { break; }
		...
		this.state = this.element.classList.contains('showing') ? 'idle' : 'readyForDeciphering';
	}
	async prepareMiniGridToDecipher(label, mnemonicStr, fakeCipher = true) {
		if (this.label !== label) { console.error('label mismatch !'); return; }
		this.state = 'prepareDeciphering';
		while (this.state !== 'idle' && this.label === label) { 
			if (!this.element.classList.contains('showing')) { this.#abortDeciphering(); return; }
		...
		for (let i = 0; i < mnemonic.length; i++) {
			const wordDiv = document.createElement('div');
			let fakeWord = lastFakeWords[i] ? lastFakeWords[i] : this.#generateRndString( mnemonic[i].length );
	...
	async decipherMiniMnemonicGrid(mnemonicStr) {
		...
		const emptyDiv = this.element.children[0];
		emptyDiv.innerHTML += `<div class="buttonsWrap active">
...
class MnemoLinkSVG {
	...
	#initLines(nbOfLines = 3) {
		...
		for (let i = 0; i < nbOfLines; i++) {
			...
			const svgLink = new SvgLink(arrayOfSVGPath, i === 0 ? 0.22 : 0);
